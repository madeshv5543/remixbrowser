...
UniversalDApp.prototype.getAccounts = function (cb) {
  ...
  if (!executionContext.isVM()) {
    ...
    if (this._deps.config.get('settings/personal-mode')) {
      ...
      executionContext.web3().adh.getAccounts(cb)
    }
  } else {
    if (!self.accounts) {
...
UniversalDApp.prototype.getBalance = function (address, cb) {
  ...
  if (!executionContext.isVM()) {
    executionContext.web3().adh.getBalance(address, function (err, res) {
      if (err) {
        cb(err)
      } else {
      ...
      self.txRunner.rawRun(tx,
        ...
        (network, tx, gasEstimation, continueTxExecution, cancelCb) => {
          ...
          var content = confirmDialog(tx, amount, gasEstimation, self,
            ...
            (cb) => {
              executionContext.web3().adh.getGasPrice((error, gasPrice) => {
